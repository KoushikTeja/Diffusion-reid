...
\begin{enumerate}
    \item \textbf{Pseudo label generation:} Features are extracted from a pre-trained backbone network, and clustering algorithms (commonly DBSCAN \cite{ester1996dbscan}) are applied to assign pseudo identity labels to each training sample.
    \item \textbf{Model training:} The model is trained using these noisy pseudo labels through classification and metric learning losses, and the process repeats.
\end{enumerate}

A key challenge in this pipeline is that pseudo labels generated from global features are inherently noisy --- samples from the same person may be split into different clusters (under-segmentation), or samples from different persons may be merged together (over-segmentation). Recent approaches attempt to mitigate this noise by incorporating fine-grained \textit{part-based features} that capture local body region information. However, existing part-based methods often compromise the discriminative semantic structure of the generated patches and overlook the consistency between local and global representations.
