...
Dai~\textit{et~al.} \cite{dai2022cluster} proposed Cluster Contrast, which stores and updates cluster-level feature representations in a memory bank. This approach enables efficient contrastive learning at the cluster level, avoiding the computational overhead of instance-level memory banks while achieving competitive performance.
...
Sun~\textit{et~al.} \cite{sun2018beyond} introduced PCB, which uniformly partitions the convolutional feature map into horizontal stripes, each representing a body part. While simple and effective, uniform partitioning does not account for pose variations and misalignment between body parts across different images.

\subsection{Spatial Transformer Networks (STN)}

Jaderberg~\textit{et~al.} \cite{jaderberg2015spatial} proposed Spatial Transformer Networks, which provide a differentiable mechanism for learned spatial transformations within neural networks. STNs learn to apply affine transformations (scaling, translation, rotation) to input feature maps, enabling attention to semantically meaningful regions. The PISL framework extends STNs by introducing a diffusion-based generative process over the spatial transformer parameters, producing patches with intrinsic semantic structure rather than relying on predefined or learned static partitions.
...
While initially developed for image generation, diffusion models have been adapted to various vision tasks including image segmentation, super-resolution, and object detection. PISL \cite{tao2024unsupervised} introduces a novel application --- generating \textit{spatial transformer parameters} through a denoising process rather than generating images directly. This represents a creative use of the diffusion paradigm for structured parameter generation beyond traditional generative modelling.

\section{Summary}

Table~\ref{tab:lit_summary} summarizes the key unsupervised Re-ID methods discussed in this chapter along with their performance on Market-1501.
...
The literature shows a clear progression toward incorporating part-based features and pseudo label refinement for improving unsupervised Re-ID. PISL advances this line of work by introducing diffusion models for semantically structured patch generation, achieving state-of-the-art results at the time of publication.
